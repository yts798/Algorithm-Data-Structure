...
bool is_single_run_possible() {
  int vertice, edge, i, start, end, vert_now, vert_next, ajc_vert, can_trim=0;
  graph_t *graph;
  node_t *temp;

  /* read in the data and save edges to adjancency list */
  scanf("%d %d",&vertice, &edge);
  vertice += 1; /* correct the total number of nodes */
  ...
  /* start depth-first search, traverse graph, mark visited nodes and produce
  the order of topological sorting */
  dfs(graph, STARTPOINT);

  /* if a tree cannot be arrived from mountain top, 
  park ranger can't ski to it */
  for(i=0;i<vertice;i++) {
    if(!graph->visited[i]) {
      return false;
    }
  ...
  /* check whether a tree can always reach the next tree directly
  ...
  for (i=0;i<vertice-1;i++) {
    ...
    if (!can_trim) { 
      return false; /* a tree cannot reach its next tree directly */
    }
    vert_now = vert_next;
    can_trim = 0;
  }
...
graph_t *new_graph(int vertice) {
  ...
  /* record topological ordering */
  graph->topo_order = (int *)malloc(vertice*sizeof(int)); 
  graph->topo_index = vertice; /* indicates the index of topo_order array */

  for (i=0;i<vertice;i++) {
    graph->visited[i] = 0; /* none node is visited yet */
    graph->adjlsts[i] = NULL;
    graph->topo_order[i] = -1; /* topological ordering hasn't produced */
  }
...
void dfs(graph_t *graph, int vertex) {
  ...
  /* Record the popped off nodes reversely by decreasing array index */
